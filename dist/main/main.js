...
electron_1.ipcMain.handle('add-discount', async (_, discount) => {
    const db = (0, database_1.getDb)();
    const result = db.prepare(`INSERT INTO discounts (name, name_ar, type, value, min_order) VALUES (?, ?, ?, ?, ?)`).run(discount.name || discount.name_ar, discount.name_ar, discount.type, discount.value, discount.min_order || 0);
    return result.lastInsertRowid;
});
...
electron_1.ipcMain.handle('create-order', async (_, order) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'paid', ?, ?)
  `);
    const result = stmt.run(order.orderNumber, order.orderTypeId || null, order.tableId || null, order.cashierId || null, order.branchId || null, order.subtotal, order.discountId || null, order.discountValue || 0, order.taxRate || 0.15, order.taxAmount || 0, order.total, order.paymentMethod || 'cash', order.notes || '');
...
electron_1.ipcMain.handle('get-quick-stats', async () => {
  ...
  `).get(today);
    ...
    FROM orders WHERE status = 'paid' AND DATE(created_at) = ?
  `).get(yesterday);
    const pendingOrders = db.prepare(`SELECT COUNT(*) as count FROM orders WHERE status IN ('pending', 'preparing')`).get();
    const lowStock = db.prepare(`SELECT COUNT(*) as count FROM products WHERE stock <= min_stock AND is_active = 1`).get();
...
electron_1.ipcMain.handle('add-product', async (_, product) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, 1)
  `).run(product.barcode || null, product.name || '', product.nameAr || product.name || '', product.price || 0, product.categoryId || null, product.image || null, product.stock || 0);
    return result.lastInsertRowid;
});
electron_1.ipcMain.handle('update-product', async (_, id, product) => {
    const db = (0, database_1.getDb)();
    db.prepare(`UPDATE products SET barcode = ?, name = ?, name_ar = ?, price = ?, category_id = ?, image = ? WHERE id = ?`).run(product.barcode || null, product.name, product.nameAr, product.price, product.categoryId, product.image || null, id);
    return true;
});
...
electron_1.ipcMain.handle('add-category', async (_, category) => {
    ...
    const result = db.prepare(`INSERT INTO categories (name, name_ar, color, icon, sort_order) VALUES (?, ?, ?, ?, ?)`).run(category.name || category.name_ar, category.name_ar, category.color || '#6366f1', category.icon || '📦', (maxOrder?.m || 0) + 1);
    return result.lastInsertRowid;
});
// ============ REPORTS ============
...
electron_1.ipcMain.handle('create-kitchen-ticket', async (_, data) => {
    const db = (0, database_1.getDb)();
    const result = db.prepare(`INSERT INTO kitchen_tickets (order_id, table_number, items, priority) VALUES (?, ?, ?, ?)`).run(data.order_id, data.table_number, JSON.stringify(data.items), data.priority || 'normal');
    return result.lastInsertRowid;
});
// ============ CASH DRAWER ============
...
electron_1.ipcMain.handle('add-cash-drawer-event', async (_, event) => {
    const db = (0, database_1.getDb)();
    const result = db.prepare(`INSERT INTO cash_drawer_events (cashier_id, type, amount, notes) VALUES (?, ?, ?, ?)`).run(event.cashier_id, event.type, event.amount || 0, event.notes || '');
    return result.lastInsertRowid;
});
